async function loadDevice(url) {
  try {
    let response = await axios.get(url + "/meta/config.json", {});
...
async function loadGithubRepos(branch) {
  try {
    let response = await axios.get(
      "https://gerrit.omnirom.org/projects/?b=" + branch + "&p=android_device",
      {}
    );
    ...
    for (var repo in s) {
      let url =
        "https://raw.githubusercontent.com/omnirom/" + repo + "/android-10";
